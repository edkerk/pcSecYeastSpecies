...
for n =a:b
    ...
    for j = 1:length(mulist)
        ...
        for m = 1:length(ratios)
            ...
            osenseStr = 'Maximize';
            name = [strrep(ERprotein{n},'new',''),'_',num2str(mu*100),'_ratio',num2str(1/ratio),'_HNG_KMX'];
            fileName = writeLP(model_tmp,mu,f,f_unmodelER,osenseStr,rxnID,enzymedataNG_all,factor_k,name);% no extra constraint
