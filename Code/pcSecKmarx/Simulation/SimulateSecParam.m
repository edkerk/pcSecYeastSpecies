...
[num_mw, raw_mw, ~] = xlsread('protein_abundance_KM.xlsx','uniprot');
...
[~,~,chap]=xlsread('TableS1_KM.xlsx','Secretory');
%%SecComplex = unique(chap(2:end,1),'stable');
SecComplex = unique(cellstr(chap(2:end,1)), 'stable');
...
for i = 1:length(enzymedataSEC.enzyme)
    ...
    subunit_count=[];
    for k = 1:length(genes_index_proteome)
        if genes_index_proteome(k)==0
            subunit_count(k) = min(pax_abundance);
        else
            subunit_count(k) = pax_abundance(genes_index_proteome(k));
        end
    end
...
pax_protein_list = strrep(pax_protein_list,'-','_');
rxnList = regexprep(rxnList, '_', '-', 1);% all protein
