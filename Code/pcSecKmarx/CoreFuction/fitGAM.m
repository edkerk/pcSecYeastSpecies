...
function model = fitGAM(model,type)
model = changeRxnBounds(model,'r_1726',-1000,'l');
model = changeRxnBounds(model,'r_1726',0,'u');

% model = changeObjective(model,'r_1912');
% optimizeCbModel(model,'max')

%Load chemostat data:
fid = fopen('chemostatDataKMX','r');
...
for i = 1:length(exp_data(:,1))
    ...
    model = changeRxnBounds(model,'r_1912',1000,'u');
    model = changeRxnBounds(model,'r_1726',1000,'u');
