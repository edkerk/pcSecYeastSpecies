function [newModel,peptide_name,rxns] = golgiProcessing_N_KM(model,peptide,peptide_org,Length_total,NG,onlyrxns)
rxns = [];
if NG >0
    ...
    reaction{2}.eq=  sprintf('%s_GNG_G1[g] + %d GDP-alpha-D-mannose[g] => %s_GNG_G2[g] + %d GDP[g]',peptide,NG,peptide,NG);
    reaction{3}.eq=  sprintf('%s_GNG_G2[g] + %d GDP-alpha-D-mannose[g] => %s_GNG_G3[g] + %d GDP[g]',peptide,30*0.26*NG,peptide,30*0.26*NG);

% %     Superglycosylation_probability = randi([1,100]);
% %     if Superglycosylation_probability <= 26
% %         reaction{3}.eq=  sprintf('%s_GNG_G2[g] + %d GDP-alpha-D-mannose[g] => %s_GNG_G3[g] + %d GDP[g]',peptide,30*NG,peptide,30*NG);
% %     else
% %         reaction{3}.eq=  sprintf('%s_GNG_G2[g] + %d GDP-alpha-D-mannose[g] => %s_GNG_G3[g] + %d GDP[g]',peptide,NG,peptide,NG);
% %     end
