...
% NOTE: For humanized glycosylation simulations, replace golgiProcessing_KM（）with golgiProcessing_humanizedN_KM.
...
%         %another pathway for misfolding degradation(the end peptide should be %s
        ...
        [model, peptide_name,rxns{12}] = golgiProcessing_N_KM(model,peptide_name,peptide,length(seq),NG,onlyrxns);

        % HNG condition (humanized glycosylation) – use the following line instead:
% %         [model, peptide_name,rxns{12}] = golgiProcessing_humanizedN_KM(model,peptide_name,peptide,length(seq),NG,onlyrxns);
