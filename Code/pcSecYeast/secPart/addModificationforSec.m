...
% NOTE: For humanized glycosylation simulations, replace golgiProcessing_N_SCE（）with golgiProcessing_humanizedN_SCE.
...
%         %another pathway for misfolding degradation(the end peptide should be %s
        ...
        [model, peptide_name,rxns{12}] = golgiProcessing_N_SCE(model,peptide_name,peptide,length(seq),NG,onlyrxns);
% %         [model, peptide_name,rxns{12}] = golgiProcessing_humanizedN_SCE(model,peptide_name,peptide,length(seq),NG,onlyrxns);
