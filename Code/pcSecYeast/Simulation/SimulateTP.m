...
% %     rxnID = 'dilute_dummy';
    ...
    for j = 1:length(mulist)
        ...
        name = [TP{i} '_',num2str(mu*100),'_SCE'];
        fileName = writeLPSCE(model_tmp,mu,f,f_unmodelER,osenseStr,rxnID,enzymedata_new,factor_k,name);% no extra constraint
...
for j = 1:length(mulist)
    ...
    name = ['ref_',num2str(mu*100),'_SCE'];
    fileName = writeLPSCE(model_tmp,mu,f,f_unmodelER,osenseStr,rxnID,enzymedata_all,factor_k,name);% no extra constraint
    allname = [allname;{fileName}];
end
% write cluster file
