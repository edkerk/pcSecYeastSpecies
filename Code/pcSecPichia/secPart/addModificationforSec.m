...
% onlyrxns = 0,1. 0 stands for that you are going to get a new model.
% 1 stands for that you only want to export the rxnID list.
% NOTE: For humanized glycosylation simulations, replace golgiProcessing_N_PP（）with golgiProcessing_humanizedN_PP.
...
geneError = [];
 [~,geneidx] =ismember(gene,protein_info(:,2));
    if geneidx ~= 0 && cell2mat(protein_info(geneidx,3)) == 1
        ...
        Cotranslation =cell2mat(protein_info(i,15));% indicate whether the peptide enters co-translational translocation pathway
        ...
        [model,rxns{1}] = translocate(model,peptide,length(seq),SP,NG,DSB,GPI,Cotranslation,onlyrxns);
...
%         %another pathway for misfolding degradation(the end peptide should be %s
        ...
        [model, peptide_name,rxns{12}] = golgiProcessing_N_PP(model,peptide_name,peptide,length(seq),NG,onlyrxns);
        %procssing in [g]: Golgi processing humanizedN
% %         [model, peptide_name,rxns{13}] = golgiProcessing_humanizedN_PP(model,peptide_name,peptide,length(seq),NG,onlyrxns);        
        [model, peptide_name,rxns{14}] = golgiProcessing_O(model,peptide_name,peptide,length(seq),OG,onlyrxns);
        
        % mature
        [model, peptide_name,rxns{15}] = mature(model,peptide_name,peptide,onlyrxns);
        
        % trnasport to other final destination(change the finale name to be original peptide_mature)
        [model, peptide_name,rxns{16}] = transportToFinal(model,peptide_name,peptide,compartment,onlyrxns);
        
%       % Subunit transport
%       [model,rxns{16}] = subunitTrans(model,peptide,compartment);
